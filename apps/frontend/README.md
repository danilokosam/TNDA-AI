...
Status: Shell + Auth, Dashboard, Upload + Processing, and Results are built, tested, and verified. Document History, the Billing UI (upgrade/manage a plan), and Settings are not started yet. See [`PROGRESS.md`](./PROGRESS.md) for the dated build log.
...
- **Document preview** (the Results page) is session-only, built and working: the just-uploaded `File` is cached in a module-level map, keyed by job id, and shown via a `URL.createObjectURL` blob through the immediate upload→results flow — the backend never persists originals, so there is nothing to preview outside that flow. Built behind a small `DocumentPreviewSource` abstraction so a persistent, backend-provided URL could replace the session-only source later without touching any UI component that consumes it.
