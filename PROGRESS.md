...
**To resume work**, the next most valuable steps in order would be: (1) register a real Stripe webhook endpoint (via `stripe listen` locally or a real deployed URL) and confirm live-delivered events sync the same way the self-signed test events did in §9, (2) extend §10's test coverage to `documents.service.ts`'s quota math and `organization.service.ts`, plus the auth-client-isolation regression test from §8, (3) add a background poller so jobs progress without a client actively polling, (4) build the teardown/cleanup step for the E2E test script, (5) if this repo's default branch is ever renamed to `main` (or a `main` branch is intentionally introduced), update `.github/workflows/ci.yml`'s trigger branches to match — right now it's `master` for real triggering, per §10.
