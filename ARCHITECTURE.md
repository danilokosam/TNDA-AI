...
| `modules/` | One folder per bounded domain context — `auth/`, `organization/`, `billing/`, `documents/`. Each module is internally layered: `*.routes.ts` (HTTP layer) → `*.service.ts` (business logic) → `*.repository.ts` (Supabase access) → `*.schema.ts` (Zod DTOs), with `*.test.ts` files colocated next to what they test. `documents/` additionally has `documents.export.mapper.ts` (the only place export code reads Azure's raw `result_json` shape, producing a provider-agnostic `ExportRecord`) and `documents.export.serializer.ts` (an `ExportSerializer` interface + format registry, `csvSerializer` the only implementation today) — see §5's Export Flow and `docs/adr/0014-document-csv-export.md`. | Keeps each domain self-contained and consistently shaped — a new contributor who understands one module's internal layering understands all four. |
...
An "Export CSV" button next to the Documents list's filter bar exports whatever the current filters (`documentType`/`search`/`dateFrom`/`dateTo`) resolve to — never an explicit row selection; there is no checkbox-based multi-select anywhere in this UI. The export is always restricted to `completed` documents server-side, regardless of any status filter the list happens to be showing. `documents.service.ts#exportDocuments` (backend) re-queries `document_jobs` via the same tenant-scoped `listDocumentJobsForOrganization` the list endpoint uses, reduces each matched job's field-correction history to its effective (reviewed) values, maps the result through `documents.export.mapper.ts` (the one place export code reads Azure's raw `result_json` shape) into a provider-agnostic `ExportRecord`, and serializes it via `documents.export.serializer.ts`'s `csvSerializer` — the only registered format today. `GET /api/v1/documents/export` runs synchronously inside the request and returns `text/csv; charset=utf-8` with `Content-Disposition: attachment`, bounded by a fixed row ceiling (`PayloadTooLargeError`/413 if exceeded, never a silent truncation) rather than a background job. On the frontend, this is the first BFF route that proxies a non-JSON body — `apiFetchFile`/`backendFetchFile`/`parseApiFileResponse` exist specifically for this, alongside (not replacing) the JSON-only `apiFetch`/`backendFetch`. Full reasoning for every one of these decisions — the mapper/serializer split, opaque-string values, one-row-per-document, mixed-type support, the synchronous/row-ceiling choice, and filter-scoped-only selection — is in `docs/adr/0014-document-csv-export.md`.
